class q_bit_array:
    ...
    def scan(self, sequences):
        for i in range(len(sequences)):
            ...
            indexes = (word_to_index(sequences[i][0:self.q]),word_to_index(reverse_complement(sequences[i][0:self.q])))
...
def word_to_index(word : str) -> int :
    ...
    for i in range(q):
        n+=dict[word[i]]*(4**(q-i-1))
    return n

def next_index(sequence : str, i : int, q : int, previous : tuple[int]) -> int :
    dict = {"A": 0, "C": 1, "G": 2, "T": 3}
    comp = {"A": "T", "T": "A", "C": "G", "G": "C"}
    n1 = 4*(previous[0] - (4**(q-1)*dict[sequence[i-1]])) + dict[sequence[i+q-1]]
...
def test_unword() :
    ...
    print(x2 + " based on previous : " + str(next_index(x3,1,3,(word_to_index(x1),word_to_index(reverse_complement(x1))))))
